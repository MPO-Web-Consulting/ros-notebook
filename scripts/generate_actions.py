...
Generate GitHub Actions workflow files for Jupyter ROS images
...
if __name__ == '__main__':
    parser = ArgumentParser(
        description='Generate GitHub Actions workflow files for Jupyter ROS images')
    # required arguments
    parser.add_argument(
        ...
        if 'Dockerfile' in files:
            images_tags.append({
                'tag': root.removeprefix(
                    args.images_dir + '/').replace('/', '-'),
                'location': os.path.join(root, 'Dockerfile')
            })
    ...
    for im in images_tags:
        ...
        vars = {
            "TEMPLATE_TAG": im['tag'],
            "TEMPLATE_DOCKER_CONTEXT": os.path.dirname(im['location']),
        }
